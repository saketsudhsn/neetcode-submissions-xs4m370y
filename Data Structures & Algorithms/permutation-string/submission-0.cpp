class Solution {
public:

    std::string getWindowFreqStr(const std::string& str)
    {
        std::vector<int> windowCharFreq(26, 0);
        for(int i = 0; i < str.size(); i++)
        {
            windowCharFreq[str[i]-'a']++;
        }

        std::string sWindowCharFreq;
        for(int i = 0; i < 26; i++)
        {
            sWindowCharFreq.append(std::to_string(windowCharFreq[i]));
        }
        
        std::cout << "sWindowCharFreq : " << sWindowCharFreq << "\n";
        return sWindowCharFreq;
    }

    bool checkInclusion(string s1, string s2) 
    {
        int windowSize = s1.size();
        if(s2.size() < windowSize)
            return false;

        
        std::unordered_set<std::string> windowFreqMap;
        windowFreqMap.insert(getWindowFreqStr(s1));

        int startIdx = 0;
        int lastIdx = windowSize - 1;
        while(lastIdx < s2.size())
        {
            std::string currStrWindow(s2.substr(startIdx, windowSize));
            if(windowFreqMap.contains(getWindowFreqStr(currStrWindow)))
                return true;
            
            startIdx++;
            lastIdx++;
        }
        
        return false;
    }
};
